...
export class ChatHandler {
  ...
  private async handleStreamResponse(
    ...
    if (validToolCalls.length > 0) {
      ...
      const finalResponse = await this.generateToolResponse(message, conversationHistory, validToolCalls, executedTools);
    ...
    return [
      {
        ...
        content: 'You are a master instructional design assistant that helps teachers weave raw lesson ideas into high-fidelity, standards-aligned blueprints. You provide clear, actionable advice and conclude every generation with a structured JSON blueprint block.'
      },
