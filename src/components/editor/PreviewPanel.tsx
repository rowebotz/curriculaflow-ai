        ...
        <div className="flex justify-between items-start">
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent className="bg-brand-black text-white p-4 w-72 rounded-none border-none shadow-sketch-lg">
                    ...
                    <a href="#" className="text-[10px] underline hover:text-brand-primary uppercase font-black font-sans focus:outline-none">Transparency & Privacy Center</a>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
          ...
          <div className="flex flex-col items-end gap-2 shrink-0">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-[9px] font-black uppercase text-brand-gray hover:text-brand-primary focus:ring-2 focus:ring-brand-primary transition-colors"
            >
        ...
        {Array.isArray(data.modules) && data.modules.length > 0 ? (
          data.modules.map((mod: any, idx: number) => {
            ...
            return (
              <article 
                key={modId} 
                className="relative pl-8 border-l-2 border-brand-black/10" 
                role="listitem" 
                ...
                <div className="bg-white border-2 border-brand-black p-6 hover:shadow-sketch-lg transition-shadow">
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-2">
                      <h4 id={headingId} className="text-xl font-black uppercase tracking-tight text-brand-black font-display">{mod.title}</h4>
                      <TooltipProvider>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button aria-label="View module design logic" className="p-1 hover:text-brand-primary focus:ring-2 focus:ring-brand-primary rounded-full transition-colors">
                              <Info className="w-4 h-4" />
                            </button>
                          </TooltipTrigger>
                          <TooltipContent className="bg-brand-black text-white p-3 text-[10px] font-bold uppercase rounded-none max-w-[200px]">
                            Backward Design: This module was architected starting with standard {mod.standards?.[0] || 'compliance'} to ensure performance outcomes drive instruction.
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                    </div>
                         ...
                         <Popover key={`${modId}-${s}`}>
                           ...
                           <PopoverContent className="bg-brand-black text-white border-none shadow-none text-xs p-4 w-72 rounded-none z-50 font-sans">
                             ...
                             <p className="leading-relaxed text-brand-gray mb-3 italic">
                               {mod.rationale?.[s] || "Mapping verified via CurriculaFlow RAG standards engine."}
                             </p>
                      ...
                      <ul className="space-y-3">
                        {Array.isArray(mod.objectives) && mod.objectives.map((obj: string, i: number) => (
                          <li key={i} className="flex gap-2 text-sm text-brand-black leading-relaxed font-medium">
                            <span className="text-brand-primary font-black" aria-hidden="true">•</span>
                            {obj}
                          </li>
                        ))}
                      </ul>
