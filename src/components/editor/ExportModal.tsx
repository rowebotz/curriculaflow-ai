...
import { Package, CheckCircle2, Copy, Download, Share2, ShieldCheck, Database, FileText, Layout, ExternalLink } from 'lucide-react';
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[600px] border-3 border-ink shadow-sketch-lg bg-paper max-h-[90vh] overflow-y-auto" aria-describedby="export-description">
              ...
              >
                ...
                <div className="w-full space-y-6">
                  <Tabs defaultValue="canvas" className="w-full">
                    ...
                    <TabsContent value="canvas" className="p-4 bg-white border-x-2 border-b-2 border-ink text-left space-y-4">
                      <div className="flex items-center gap-2 border-b border-ink/10 pb-2">
                        ...
                        <span className="text-[10px] font-black uppercase text-brand-gray tracking-widest">Canvas Modules View</span>
                      </div>
                      <div className="p-3 border-l-4 border-[#F5F5F5] bg-white shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-muted-foreground">100 pts | LTI 1.3 External Tool</p>
                          </div>
                        </div>
                        <span className="text-[9px] bg-[#E8F5E9] text-[#2E7D32] px-2 py-0.5 font-bold rounded-full">PUBLISHED</span>
                      </div>
                    </TabsContent>
                    <TabsContent value="google" className="p-4 bg-white border-x-2 border-b-2 border-ink text-left space-y-4">
                      <div className="flex items-center gap-2 border-b border-ink/10 pb-2">
                        ...
                        <span className="text-[10px] font-black uppercase text-brand-gray tracking-widest">Classwork Stream Post</span>
                      </div>
                      <div className="p-4 border-2 border-[#E0E0E0] rounded-lg">
                        <p className="text-sm font-medium mb-3">CurriculaFlow posted a new assignment: {lessonTitle}</p>
                        <div className="flex items-center gap-3 p-2 bg-[#F8F9FA] border border-[#DADCE0] rounded cursor-pointer hover:bg-muted">
                          ...
                          <span className="text-xs font-medium text-brand-primary truncate">{lessonTitle} (LTI Launch)</span>
                        </div>
                      </div>
                    </TabsContent>
                  </Tabs>
                  <div className="p-4 bg-white border-2 border-ink text-left space-y-2">
                    <span className="text-[10px] font-bold uppercase text-muted-foreground">Sync Capabilities:</span>
                    ...
                    <div className="pt-2">
                      <div className="flex justify-between text-[10px] font-bold mb-1 uppercase">
                        <span>LTI Advantage Handshake...</span>
                  ...
                  <div className="bg-white border-2 border-ink p-4 text-left shadow-sketch-hover">
                    <label htmlFor="launch-url" className="text-[10px] font-bold uppercase text-muted-foreground block mb-1">Standard LTI Launch URL</label>
                    <div className="flex gap-2">
                      <code id="launch-url" className="bg-muted px-2 py-1 text-xs flex-1 truncate select-all">https://engine.curricula.flow/lti/v1p3/{lessonTitle.toLowerCase().replace(/\s+/g, '-')}</code>
                      <button 
                        onClick={() => handleCopy('https://engine.curricula.flow/lti/v1p3/sample')} 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <button aria-label="Push directly to Schoology" className="btn-sketch text-xs py-3 focus:ring-2 focus:ring-brand-primary">
        ...
        <DialogFooter className="sm:justify-center">
          ...
          >
            {isComplete ? "Return to Weaver" : "Cancel Export"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
